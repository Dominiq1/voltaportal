...
import MultiColorMaps from '@/components/MultiColorMaps'
import { useQuery } from '@apollo/client'
import { Box, Grid } from '@mui/material'
import requestData from '@/API/Quickbase'
import { useEffect, useState } from 'react'
//import { GET_MAP_DATA } from '@/gql/mutations/InstallMap'
import { GET_Texas_MAP_DATA } from '@/gql/mutations/InstallMap'
...
export default function Home() {
  ...
  const { loading, error, data } = useQuery(GET_Texas_MAP_DATA);
  ...
  const markers = loading ? [] : (data?.GetTexasMapData ? formatDataForMarkers(data.GetTexasMapData) : []);
  ...
  return (
    <>
      ...
      <Box sx={{ height: '100vh', width:'100vw', backgroundColor: 'black', p: 2, color: 'black' }}>
        <MultiColorMaps markers={markers} />
      </Box>
