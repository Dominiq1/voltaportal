...
import {  GET_CONSTRUCTION_MAP_DATA } from '@/gql/mutations/UserMapInfo'
...
export default function Home() {
  ...
  //  const recordID = "4061"; // Example recordID, replace with the actual value or state as needed
    ...
    const repID = router.query.recordID; // Step 3: Extract recordID from the URL query parameters
    const { id } = router.query;

    const { loading, error, data } = useQuery(GET_CONSTRUCTION_MAP_DATA, {
        variables: {repID: id }
    });
  ...
  const markers = loading ? [] : (data?.GetConstructionUserMapInfo ? formatDataForMarkers(data.GetConstructionUserMapInfo) : []);
